...
function drawPacMan() {
    ...
    if (pacDirection === "right") {
        ctx.arc(x, y, pacSize / 2, pacMouthOpen ? 0.2 : 0.4, pacMouthOpen ? 1.9 : 2.1);
    } else if (pacDirection === "left") {
        ctx.arc(x, y, pacSize / 2, pacMouthOpen ? 1.2 : 1.4, pacMouthOpen ? 2.9 : 3.1);
    } else if (pacDirection === "up") {
        ctx.arc(x, y, pacSize / 2, pacMouthOpen ? 1.7 : 1.9, pacMouthOpen ? 0.1 : 0.3);
    } else if (pacDirection === "down") {
        ctx.arc(x, y, pacSize / 2, pacMouthOpen ? 0.7 : 0.9, pacMouthOpen ? 2.1 : 2.3);
    }
...
function movePacMan() {
    const newRow = pacRow + (pacDirection === "down" ? pacSpeed : pacDirection === "up" ? -pacSpeed : 0);
    const newCol = pacCol + (pacDirection === "right" ? pacSpeed : pacDirection === "left" ? -pacSpeed : 0);
...
function moveGhost() {
    const newRow = ghostRow + (ghostDirection === "down" ? ghostSpeed : ghostDirection === "up" ? -ghostSpeed : 0);
    const newCol = ghostCol + (ghostDirection === "right" ? ghostSpeed : ghostDirection === "left" ? -ghostSpeed : 0);
...
function checkCollisions() {
    ...
    for (let i = 0; i < pellets.length; i++) {
        ...
        const distance = Math.sqrt((pacCenterX - pelletX) ** 2 + (pacCenterY - pelletY) ** 2);
    ...
    for (const cherry of cherries) {
        ...
        const distance = Math.sqrt((pacCenterX - cherryX) ** 2 + (pacCenterY - cherryY) ** 2);
    ...
    const distanceToGhost = Math.sqrt((pacCenterX - ghostCenterX) ** 2 + (pacCenterY - ghostCenterY) ** 2);
...
function resetGame() {
    ...
    cherries.length = 0;
    initializePellets();
    cherries.push({ row: 7, col: 14 }, { row: 7, col: 15 }, { row: 15, col: 7 }, { row: 15, col: 8 }, { row: 15, col: 9 });
}
